...
const MeetingCart = ({ title, date, icon, previousMeeting, buttonIcon1, buttonText, handleClick, link }: meetingCartProps) => {
      ...
      <article className={cn("flex justify-center relative", {})}>
        <div className="relative flex w-full max-sm:hidden">
          {avatarImages.map((img, i) => (
            <Image key={i} src={img} alt="attendees" width={40} height={40} className={cn("rounded-full", { absolute: i > 0 })} style={{ top: 0, left: i * 28 }} />
          ))}
