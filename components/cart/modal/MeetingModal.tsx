    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <div className="flex flex-col gap-6">
          ...
          <h1 className={cn("text-3xl font-bold leading-[42px] mb-2 text-white")}>{title}</h1>

          {children}

          <Button className="bg-blue-1 focus-visible:ring-0 focus-visible:ring-offset-0 text-white" onClick={handleClick}>
            {buttonIcon && <Image src={buttonIcon} alt="button icon" width={13} height={13} />} &nbsp;
            {isLoading ? "Loading..." : buttonText || "Schedule Meeting"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
